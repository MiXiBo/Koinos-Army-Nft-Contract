...
export class Collections {
  ...
  mint_start_time(args: collections.mint_start_time_arguments): collections.uint64_object {
    return new collections.uint64_object(Constants.START_TIME);
  }

  mint_started(args: collections.mint_started_arguments): collections.bool_object {
    const blocktime = System.getHeadInfo().head_block_time;
    return new collections.bool_object(blocktime >= Constants.START_TIME);
  }
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    // early bird access for Koincity Moon Boys and OG Rex holders
    const moonBoys = new Nft(Constants.NFT_KCMB);
    const mBalance = moonBoys.balance_of(new collections.balance_of_arguments(to)).value;
    const ogRex = new Nft(Constants.NFT_REX);
    const rBalance = ogRex.balance_of(new collections.balance_of_arguments(to)).value;

    if(mBalance === 0 && rBalance === 0) {
      System.require(blocktime >= Constants.START_TIME, "Mint not started");
    }
    ...
    if (Constants.MINT_FEE > 0) {
      System.requireAuthority(authority.authorization_type.contract_call, to);
